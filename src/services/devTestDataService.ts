...
function createPerfectDefaults(now: Date): TradeProfile {
  ...
  return {
    ...
    companyName: '테스트수출 주식회사',
    companyAddress: '부산광역시 중구 중앙대로 1',
    companyCountry: '대한민국',
...
export function createRevisionTestProfile(currentProfile: TradeProfile, now = new Date()): TradeProfile {
  ...
  return {
    ...filled,
    itemName: hasValue(currentProfile.itemName) ? currentProfile.itemName : '테스트 화물',
    companyName: hasValue(currentProfile.companyName) ? currentProfile.companyName : '테스트업체',

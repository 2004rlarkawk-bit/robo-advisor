...
export function renderTransportRequestHTML(data: TransportRequestData): string {
  const itemRows = data.items.map((item, index) => `
    <tr>
      <td>${index + 1}</td><td style="text-align:left">${show(item.marksAndNumbers || data.shippingMarks)}</td><td style="text-align:left">${show(item.description)}</td>
  ...
  const freightTerms = data.freightTerms || 'TBD';
  ...
  return `
    <div style="width:820px;max-width:100%;margin:0 auto;background:#fff;color:#172033;font-family:Arial,'Noto Sans KR',sans-serif;padding:36px;box-sizing:border-box">
      ...
      <table class="tr-table"><thead><tr><th>No.</th><th>Marks &amp; Numbers</th><th>Goods Description</th><th>HS Code</th><th>Qty</th><th>Unit</th><th>Pkg Qty</th><th>Package Type</th><th>N/W (kg)</th><th>G/W (kg)</th><th>CBM</th></tr></thead><tbody>${itemRows || '<tr><td colspan="11">No cargo item entered</td></tr>'}</tbody></table>
      <div class="tr-grid">
        ...
        <div class="tr-box"><span class="tr-label">Transport Request</span>Place of Receipt: ${show(data.placeOfReceipt)}<br>POL: <b>${show(data.loadPort)}</b><br>POD: <b>${show(data.dischargePort)}</b><br>Place of Delivery: ${show(data.placeOfDelivery)}<br>Requested Departure: ${show(data.requestedDepartureDate)}<br>Loading Mode: <b>${show(loadingMode)}</b> &nbsp; Freight: <b>${show(freightTerms)}</b></div>
      </div>

import type { ImportComparisonRow } from '../../types/importTrade';
import { comparisonOutliers } from '../../utils/comparisonOutliers';

export default function ImportDocumentComparison({ rows, title = '문서 간 대사' }: { rows: ImportComparisonRow[]; title?: string }) {
  return (
    <section className="form-card import-card">
      <div className="import-card-heading"><div><h2>{title}</h2></div><p>AI가 B/L·C/I·P/L에서 같은 값을 추출해 자동 대조한 결과입니다. 불일치 항목은 서류 검토 사항에 이슈로 등록됩니다.</p></div>
      {rows.length === 0 ? <p className="form-message" role="status">아직 서류 간 비교 결과가 없습니다. 결과가 없다고 해서 모든 항목이 일치하는 것은 아닙니다.</p> : <>
      <div className="import-table-wrap">
        <table className="import-table">
          <thead><tr><th>검사항목</th><th>C/I</th><th>P/L</th><th>B/L</th><th>결과</th></tr></thead>
          <tbody>{rows.map((row) => {
            const outliers = comparisonOutliers(row);
            return <tr key={row.field}>
              <th>{row.field}</th>
              <td className={outliers.has('invoice') ? 'mismatch-value' : undefined}>{row.invoice}</td>
              <td className={outliers.has('packingList') ? 'mismatch-value' : undefined}>{row.packingList}</td>
              <td className={outliers.has('billOfLading') ? 'mismatch-value' : undefined}>{row.billOfLading}</td>
              <td><span className={`match-badge ${row.matches ? 'match' : 'mismatch'}`} title={row.detail}>{row.matches ? '일치' : '불일치'}</span></td>
            </tr>;
          })}</tbody>
        </table>
      </div>
      {rows.every((row) => row.matches) && <div className="form-message success">비교된 {rows.length}개 항목이 모두 일치합니다.</div>}
      </>}
    </section>
  );
}

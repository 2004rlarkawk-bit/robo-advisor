 ...
 * - 실제 DOM 요소를 텍스트로 찾아 하이라이트하므로 App.tsx의 요소를 수정할 필요가 없다.
 * - 대상이 화면에 없으면(예: 작업실 밖) 해당 단계는 자동으로 건너뛴다.
...
const STEPS: Step[] = [
  {
    ...
    desc: '통관 작업실에서 거래 정보를 입력하고 서류를 만들어요. 대부분의 작업이 여기서 이뤄집니다.',
  },
  {
    find: () => findButtonByText('필요 서류 자동 생성'),
    chip: 'STEP 2',
    title: '버튼 하나로 서류 자동 생성',
    desc: 'AI 에이전트가 필요한 서류를 판별하고, 문서를 만들고, 통관 규정에 맞는지 검증해줘요.',
  },
  {
...
export default function OnboardingTour() {
  ...
  const [tick, setTick] = useState(0); // 리사이즈 등으로 재계산 유도

  // 다음(dir=1)/이전(dir=-1) 방향으로 화면에 실제로 존재하는 단계를 찾는다.
  const nextValid = useCallback((from: number, dir: number): number => {
    for (let i = from; i >= 0 && i < STEPS.length; i += dir) {
      if (STEPS[i].find()) return i;
    }
    return -1;
  }, []);

  // 최초 마운트 시: 이미 봤으면 표시 안 함. 아니면 DOM이 준비된 뒤 첫 유효 단계부터 시작.
  useEffect(() => {
    if (localStorage.getItem(STORAGE_KEY)) return;
    const t = setTimeout(() => {
      const first = nextValid(0, 1);
      if (first === -1) return; // 짚을 대상이 하나도 없으면 조용히 종료
      setIdx(first);
      setActive(true);
    }, 700);
  ...
  const PAD = 8;
  const isFirst = nextValid(idx - 1, -1) === -1;
  const isLast = nextValid(idx + 1, 1) === -1;

  // 말풍선 위치: 대상 오른쪽에 공간이 있으면 오른쪽, 없으면 아래
  const TIP_W = 300;
  const placeRight = box.left + box.width + 20 + TIP_W < window.innerWidth;
  const tipStyle: React.CSSProperties = placeRight

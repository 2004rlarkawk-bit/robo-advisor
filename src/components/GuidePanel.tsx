...
const EXPORT_FLOWS: RoleFlow[] = [
  {
    ...
    steps: [
      ...
      { title: '제출 · 포워더 의뢰', desc: '문서를 최종 제출한 뒤 [포워더 의뢰]에서 담당 포워더를 지정해요.' },
    ],
  },
  {
    role: '포워더',
...
const IMPORT_FLOWS: RoleFlow[] = [
  {
    ...
    steps: [
      ...
      { title: '제출 · 포워더 의뢰', desc: '완성한 서류를 최종 제출한 뒤 [포워더 의뢰]에서 담당 포워더를 지정해요.' },
    ],
  },
  {
    role: '포워더',
...
const FAQ_GROUPS: { key: string; label: string; items: { q: string; a: string }[] }[] = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        a: '프로필의 서비스 이용 목적을 [관리자]으로 저장하면, AI 통관 작업실에서 그때그때 역할을 고를 수 있어요.',
      },
      {
  ...
  },
  {
    ...
    items: [
      {
        q: 'HS CODE를 몰라요.',
        a: '품명만 구체적으로 적으면 관세청 HSK 목록에 있는지 확인해 후보를 추천해요. 상세정보를 채우고 [HS 재추천]을 누르거나, 소호를 골라 범위를 좁히거나, 아는 코드를 직접 입력해도 됩니다.',
      },
      {
        q: '서류는 어떤 파일로 받나요?',
